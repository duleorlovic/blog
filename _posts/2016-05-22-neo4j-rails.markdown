...
Debug with `user.assets.categories.assets.print_cypher` or some query `move.query_as(:m1).match('(m1)-[]-(g:Group)').to_cypher`
...
# find_by supports only single value event it is has_many
...
node you can use string like `where("node.uuid = #{n.id}")` or hash `where(node:
{ neo_id: n.id })` or `match_nodes(node: n)`
