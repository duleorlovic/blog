...
Show remote show origin url

    git config --get remote.origin.url
    # or copy to clipboard using pbcopy
    git remote-copy
...
To move commit to stage (it is similar to amend, but you have commit files on
stage so you can restore --staged changes to unstage them)
...
Permission problem is when you install gh cli using snap. Solved with installing
with apt-get.
