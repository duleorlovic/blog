...
* `+` and `-` move on the first letter up and down
* `{` and `}` move on paragraph
...
* mA then 'A jump there (downcase letter a can jump only on current buffer),
  :marks gives the jump location, '' is jump back, '. jump to last change
...
* recording macro to q with `qq`, exit with `q`, run with `@q`, paste commands
  in insert mode with `^Rq`
