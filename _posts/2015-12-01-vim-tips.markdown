...
* `<c-d>` is control Ctrl and d, in help it can be written as `CTRL-D`
* `<esc>` is escape key (when recording macro it is `€ý`)
* `<space>` space
...
Use Ctrl-o to run normal mode command from insert Mode (instead of `^o` I
write `ctrl-o` or `<c-o>`).
* `<c-o>` take to normal mode for just ONE command, so if you want to
  go up while in insert mode `<c-o> k`, or to reformat paragraph `<c-o> gqip`,
  clear whole line but not new line `<c-o><s-o>` (similiar to `<c-u>`
...
* `<c-h>` backspace
* `<c-j>` return
* `<c-a>` insert the text you typed last time you were in Insert mode
* `<c-p>` complete words under the cursor. If there is no word, it look previous
  word, `<c-p>` again look previous...

# Operator

* to change case you can select and press `~`
* `gq` text formatting (`gw` formating without cursos movement)
...
* tab completion works and you can see all matches with `CTRL-D` (match is
  when command starts or short name starts with that). `^p` in reverse
  direction. To get a value use `:set isk<TAB>=<TAB>`
...
* `cd ~` to change directory to home folder for all windows. If you use `gf` in
  ...
  same folder). To manually change current directory to file path you can use
  `:lcd %:p:h` (% is the name, %:p file full path, %:p:h give its directory)
...
* `mA` then `'A` jump there (downcase letter `a` can jump only on current file
  buffer), `:marks` gives the jump location, two ' `''` is jump back, `'.` jump
  to last change
...
* `[l` `]l` ALE errors. To close error window you need to `:lcl`. I use map for
  quickfix to `:.cc`
* `]b` buffers
...
* undo `u` redo `c-r`. If you know the change number you can jump to it using
  `:undo 2` . Use `g+` and `g-` to jump back and see change number so you can
  jump to it. See change numbers with `:UndotreeShow`
...
* another end of word is `\>` so you can search `/usan\>`
...
* replace tabs with two spaces in whole file(you can use % also) `:1,$s/\t/  /g`
  range from current line to the end is `:.,$s/yes/no/`. This is example to
  replace grey with gray only in current chapter (each chapter starts with
  Chapter) search backward from current position use `?`
  ```
...
dates
* for commands you can use `^d` to show all completion and `<tab>` to iterate
  over it, `^p` to iterate in reverse direction
...
Read all `:help usr_toc.txt` last is usr_24.txt
For functions use [vim theory]({{ site.baseurl }} {% post_url 2017-06-05-vim-theory %})
