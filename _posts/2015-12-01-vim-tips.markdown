...
* `<c-o>` is ctrl-o, take to normal mode for just ONE command, so if you want to
  go up in insert mode `<c-o> k`, or to reformat paragraph `<c-o> gqip`
...
apt-get install vim-gtk`. or `vim-gnome`. On ubuntu 18 that package is outdated
...
* `cd ~` to change directory to home folder. If you use `gf` in Gemfile.lock
  (not in Gemfile), it will change directory only for that window (you can check
  with `:pwd`). To manually change current directory to file path you can use
  `lcd %:p:h` (% is the name, %:p five full path, %:p:h give its directory)
...
* `:argdo %s/foo/bar/ge | update`  replace (substitute) in all arg. replace tabs
  with two spaces in whole file `:1,$s/\t/  /g`
