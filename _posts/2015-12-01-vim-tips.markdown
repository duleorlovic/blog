...
Instead of `CTRL + o` I would write `^o`...

Some keys:
* `<c-o>` is ctrl-o, take to normal mode
* `<s-o>` (i don't know)
...
yank link to register `a` and paste: `"add` `"ap"`
...
* `]b` buffers
* `]q` search Quick fix
others are in `:help unimpaired-next`
To close error window you need to `:lcl`
...
There was issue with erb <https://github.com/w0rp/ale/issues/580>.
...
You can jump to next error or warning with `:lnext` or `:lpr` or using `[l` `]l`
...
`rubocop --auto-correct Gemfile`
  * [.rubocop.yml](https://github.com/duleorlovic/config/blob/master/.rubocop.yml)
  in $HOME or project root. You can disable `AllCops` or specific file paths,
  for example BlockLength for rspec files
...
Place language specific (file type) plugins inside `~/.vim/ftplugin/ruby.vim`

Use https://github.com/janko-m/vim-test and short cuts `<leader>t` test nearest
spec to the line (if not inside example, whole file will be run). Also
* `<leader>T` test whole file
* `<leader>a` test whole suite
* `<leader>l` test last, usefull if you navigate to the code and see if test
pass
* `<leader>g` visit test, jump back to the test to write more tests

I use strategy `let test#strategy = "dispatch"`. Asyncrun will not collectly
parse errors:
