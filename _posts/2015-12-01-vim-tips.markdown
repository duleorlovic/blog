...
Control key is upper caret `^` so instead `Ctrl + n` it can be written as `^n`.
...
* using vim inside irb or rails console with gem
  ...
  can write your input data in file/ruby code and insert byebug/debuger/pry in
...
* `^n` or `^p` to autocomplete, `^x^n` only from current file`, `^x^f` to
  complete filename (keep `^x^f` when you navigate subfolders), `^x^]` for tags
  find other shortcuts with `:help ins-completion`
* `set showcmd` enable showing all keys that I'm typing in bottom right corner,
  very usefull when explaining to someone
* snippet `nnoremap ,html :-1read $HOME/.vim/.skeleton.html<CR>3jwf>a`
* snippet `nnoremap ,baseurl :-1read $HOME/config/vim/jekyll.baseurl.md<CR>'
