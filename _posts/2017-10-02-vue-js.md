...
In components use `<script>` than `<template>` then `<style>` (since script is
usually more updated than template).
...
const app = new Vue({
  el: '#hello',
  store: store,
  // instead of using full store.state.count
  // computed: {
  //   count () {
...
Actions are called with `.dispatch` and also accepts payload and all
...
```
Validation can be implemented with Vuelidate https://bootstrap-vue.org/docs/reference/validation
