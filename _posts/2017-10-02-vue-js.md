...
* For html attributes you have to use `<div v-bind:id='dynamicId'></div>`. When
  ...
  value will show it. When value is `false`,
...
* model `v-model='message'` is used for two way binding. `v-bind='message'` is
  ...
  Text and textarea uses `value` property and `input` event (use modifier
  `v-model.lazy='msg'` if you want to sync after `change` event), checkbox and
  radiobutton use `checked` property and `change` event, select field use
  `value` property and `change` event. It ignores initial `value` attributes,
  and uses vue instance data. Modifiers: `.number` typecast to Number, `.trim`
  strip whitespace.
  ```
  <p>{{ message }}</p>
  <input v-model="message">
...
* use `<div ref='my-ref'>` to reference using `this.$refs['my-ref'].focus()`
  ...
  showed in DOM, vue will use `data-v-123asd` attribute).
...
* component name should be multi-word, component data should be function,
...
so better is to use one common store using vuex. Store has: state, mutations
and actions.
Inside components you can access store using `this.$store` attribute or better
is using `mapState` which enables arrow functions, can access state with list of
names so no need for `this.$store.state.count`.  Similarly `mapGetters` is used
for getters, `mapMutations` for `.commit` and `mapActions` for `.dispatch` (they
requries root `store: store` injection).
...
second parameter `getters`). Mutations are similar to events. *event
registration* is using name ie **type** and **handler** function that receive
`state` param. Second parameter can be **payload** (used as argument when
commiting a mutation). Since state is reactive, when we mutate the state,
components observing the state will update automatically. Mutation are
synchronous transactions, to handle asynchronous, use Actions (which use
Mutations). Actions are called with `.dispatch` and also accepts payload and all
params can be one object `store.dispatch(type: 'increment', amout: 10)`
...
const store = new Vuex.Store({
  ...
  actions: {
    // invoke with store.dispatch('increment')
    //
...
// if namespaces: true than mutations, getters and actions are not global and we
// need to use path (for getters use square brackets)
