...
* For html attributes you have to use `<div v-bind:id='dynamicId'></div>`. When
  html atribute is boolean (it's presence is important, not value) than `true` value will show it. When value is `false`,
...
Listening to child components events is vith `v-on='name-of-event'` and child
should `$emit('name-of-event')`. Passing variable is catched using `$event` or
in case of function, it is first parameter.
