...
[list of keysym](http://wiki.linuxquestions.org/wiki/List_of_KeySyms) AltGr,
Alt_R, AltGr_R are synonyms.
...
To find keycodes use `xev` program (exit with mouse or alt+f4). `showkeys` is
using for linux system. You can use also `xkeycaps` but I do not know which
keyboard to choose.
Keycode for `a` is 64, and keysym is value assigned to that keycode `0x61` (ie
`a`). Keysym for `shift+a` is `0x41` ie `A`.
...
windows 133 (keysym `Super_L`), caps lock 66.

To see all modifier and keysym binded to them:
~~~
xmodmap -pm
