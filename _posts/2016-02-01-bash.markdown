...
```
Login shell is running `/etc/profile` and `~/.bash_profile`, `~/.bash_login` and
`~/.profile` (first found file is sourced, others are ignored)

Interactive shell is running `~/.bashrc`
I usually `source ~/.bashrc` from `~/.bash_profile` so login shell become also
interactive shell.
...
Interesting [devtools-cheatcheet](http://anti-code.com/devtools-cheatsheet/)
...
* input first param `$1`. `$@` is all params. Default value with `:-` for
  example `local port="${1:-8000}"`
  Use `"$@"` if you pass to another script and you want to preserve quoted
  strings
  ...
  around with quotes, for example `A="file with space.txt"; ls "$A"` There is
  also a `$*` which is very similar to `$@` but when used by quote `"$*"` it is
  joined by IFS character `"$1c$2c$3c…c${N}"` so here is function
...
* `ps aux | grep process_name` will return always the grep process because it
  ...
  that is listening on a port ports list
...
* get folder from path `echo $(dirname "${f}")` file `basename "asd/a.t"` or
  last folder (after last /) from current folder `basename $(pwd)`
...
* test the speed on ssh on remote server:
  ...
  * upload: generate large file `fallocate -l 1G gentoo_root.img` and use scp to
  test upload link. create big file
...
```
Exit ssh session when there are connection issues and ssh becomes unresponsive
stale, you can can exist with tilda and dot
```
<Enter>
~.
```
...
* check if file contains string and run code only if does not include string
  ...
  ```
* `telnet host port` is usefull to check if it is open port, escape character is
  ctrl + ]
  ```
...
* send email from bash
  ...
  useSTARTTLS=YES
  AuthUser=duleorlovic@gmx.com
  AuthPass=my-passs
  TLS_CA_File=/etc/pki/tls/certs/ca-bundle.crt
  ```
  when sending from username that is different we need to define `From:` and
  separate with two lines `\n\n`
  ```
  echo -e "From: duleorlovic@gmx.com\nSubject: My subject\n\nTest message body ssmtp" | ssmtp -vvv duleorlovic@asd.com
  ```
  ...
  if ~/my-app/my-app/tmp/a.sh | grep -q DS ; then
    ...
    if [ ! -f ~/my-app/my-app/tmp/a.sh.stop ]; then
      echo "From: duleorlovic@gmx.com\nSubject: my-app login from mail failed\n\nTo continue with this notification you need to run\nrm ~/my-app/my-app/tmp/a.sh.stop" | ssmtp -vvv duleorlovic@asd.com >> ~/my-app/my-app/tmp/a.sh.log 2>&1
...
Find info with `man screen` or online https://www.gnu.org/software/screen/manual/screen.html
...
* all session `screen -ls` start new named session `screen -S dule`, exit
detach with `c-a d` and reattach with `screen -x` or `screen -r dule`. Close
current region `c-a X` (uppercase X) close all except current `c-a Q`.
kill current `c-a k`, kill all with `c-a \`.
* create new window `c-a c` and switch to it using `c-a 1`, list all
windows `c-a "` you can switch to it by jk and enter, show window bar `c-a w`,
go to next/prev window `c-a p` or `c-a <space>`, `c-a A` rename window
* split `c-a |` and move to that region `c-a <tab>`, close all splits except
current `c-Q` (you can also `:split`)
* `c-a [` (or `c-a <esc>`) to go to copy mode, press space or enter to mark and
`c-a ]` to paste
* to wrap you need to enable `c-a:wrap on`
* since screen will catch ctrl+c so to get to shell in window where some process
...
* to send ctrl+C to process you can remap `stty intr ^]` so you can use `ctrl+]`
to kill the process but keep window and shell running
https://unix.stackexchange.com/a/171707/150895

* change escape key to j https://wiki.archlinux.org/title/GNU_Screen#Change_the_escape_key
  https://stackoverflow.com/a/7251588/287166
* multiuser https://unix.stackexchange.com/questions/163872/sharing-a-terminal-with-multiple-users-with-screen-or-otherwise
set up the server for ssh access
```
sudo useradd -m help --shell /bin/bash
sudo passwd help
...
SESSION_NAME="help_session"
COMMAND_TO_RUN="ssh -R 2222:localhost:22 power"

# Check if the screen session already exists
if screen -list | grep -q "\.${SESSION_NAME}"; then
    echo "Screen session '${SESSION_NAME}' already exists, try: screen -r help"
else
    # Create a new detached screen session and run the command
    screen -dmS "${SESSION_NAME}" ${COMMAND_TO_RUN}
    echo "Screen session '${SESSION_NAME}' started and command executed."
    screen -S $SESSION_NAME -X screen -t "Bash 1" bash -c "export PS1='help \w\$ '; exec bash"
