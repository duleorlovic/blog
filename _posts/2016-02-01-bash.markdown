...
~~~
* screen command
  * list all session `screen -list` start new named session `screen -S dule`
  * create new window `Ctrl+A + c` and switch to it using `Ctrl+A + 1`
  * list all windows `Ctrl+ "` you can switch to it by jk and enter
  * exit detach with `Ctrl+A + d` and reattach with `screen -r` or you can exit
    with `ctrl+a` followed by `ctrl + \`
