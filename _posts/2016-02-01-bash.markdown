...
* `xwininfo` to click on window to find it's `0x440003d` which we can use to
  ...
  Name: *main editor* command `xdotool windowsactivate 0x123123` shortcut
  *Alt+M*.
...
* screen command
  * `screen -r` to reattach to window (which you exited with Ctrl+A+D). Ctrl+A+]
  to go to copy mode.
* send desktop notifications with `notify-send` but need urgency critical, so I
made alias `alert`
* chearsheet https://devhints.io/bash
