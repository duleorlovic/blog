...
* destructuring with default parameters values and renaming
  ...
  For array you can clone (duplicate) with `const cloned = [...array]`
...
* `for(i=0;i<cars.length;i++) {}` is not concise `cars.forEach(myFunction)` is
  ...
  (for values in array or string) and `for..in` (for object property names) is
  concise and can break. `for in` iterates over enumerable properties of an
  object in arbitrary order, `for of` iterates over values of iterable object.
  Use `.entries()` to get index
  ```
  ...
  To convert HtmlCollection to array you can use `let array =
  Array.from(htmlCollection)` and `array.forEach(item) => { console.log(item) }`
...
* `[1,2,3].filter( (n) => n > 2 )`. This can be used as `.compact` in ruby for
