...
* finding elements by class or id or any css (instead of
  `getElementsByClassName('class)')` which return `HTMLCollections` which is
  live, so you have to convert to array `Array.from(elements)`) you should use
