...
and add this initialization file (similar to autogenerated with `rails g
...
if (receivers = Rails.application.secrets.exception_recipients).present?
  ExceptionNotification.configure do |config|
    ...
    ignore_crawlers(config)

    ignore_formats(config)
  ...
  def ignore_crawlers(config)
    ignored_http_user_agents = %w(Googlebot bingbot linkdexbot Baiduspider
                                  YandexBot panscient.com MJ12bot).freeze
    config.ignore_if do |_exception, options|
      options[:env] && Array(ignored_http_user_agents).any? do |crawler|
        options[:env]['HTTP_USER_AGENT'] =~ Regexp.new(crawler)
      end
    end
  end

  def ignore_formats(config)
    # Ignore formats. chrome on windows sometimes asks for bitmap on root url
  ...
  def throttle(config)
    ...
    throttle_limit = 3 # this resets only when nothing is seen in interval window
    throttle_interval_seconds = 1.hour

    config.ignore_if do |exception|
      cache_key = exception.message
      # ignore eventual object hex id so we treat as same message
      cache_key.sub!(/0[xX][0-9a-fA-F]+/, '')
      already = Rails.cache.fetch(cache_key)
      if already
        Rails.cache.write cache_key, already + 1, expires_in: throttle_interval_seconds
...
class PagesController < ApplicationController
  ...
  def sample_error_in_resque
    ...
    render text: "TaskWithError in queue, please run resque:work"
  end
end
...
Notifications in resque could be generated with `rails g
exception_notification:install --resque` or simply add those lines
